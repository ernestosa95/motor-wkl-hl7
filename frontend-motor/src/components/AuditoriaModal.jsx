...
import { X, LogIn, AlertTriangle, CheckCircle2, FileText } from 'lucide-react';
...
export default function AuditoriaModal({ orden, onClose }) {
  ...
  return (
    // overflow-y-auto en el overlay + items-start: el modal se alinea arriba y
    // el scroll es de toda la capa, así el encabezado nunca queda cortado.
    <div
      className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/40 p-4 sm:p-8"
      onClick={onClose}
    >
      <div
        className="w-full max-w-lg rounded-xl bg-white shadow-xl my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Encabezado fijo dentro del modal */}
        <div className="sticky top-0 z-10 flex items-center justify-between rounded-t-xl border-b border-zinc-100 bg-white px-5 py-4">
          <div>
          ...
          <div className="p-5 flex flex-col gap-5">
            ...
            {data.detalles_error && (
              <div className="rounded-lg border border-rose-200 bg-rose-50 p-3">
                ...
                <p className="text-sm text-rose-800 break-words">{data.detalles_error.mensaje}</p>
...
function Bloque({ titulo, contenido }) {
  // Cada segmento HL7 en su propia línea para legibilidad (separador |\r o salto real)
  ...
  return (
    <div className="mb-2">
      ...
      <pre className="overflow-x-auto rounded-lg bg-zinc-900 p-3 text-[11px] leading-relaxed text-zinc-100 whitespace-pre-wrap break-words">
        {texto}
      </pre>
    </div>
  );
